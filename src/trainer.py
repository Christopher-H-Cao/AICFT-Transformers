...
class Trainer(object):
    def __init__(self, modules, env, params):
        ...
        if params.amp >= 0:
            ...
            #without amp, training is sometimes unstable, so kill it if we get a NaN
        ...
        self.metrics_to_checkpoint = []
        ckpt_metrics = [m for m in params.checkpoint_best_metrics.split(",") if m != ""]
        for m in ckpt_metrics:
            m = (m[1:], False) if m[0] == "_" else (m, True)
            self.metrics_to_checkpoint.append(m)

        for (metric,big) in self.metrics:
            for (metric_ckpt,big_ckpt) in self.metrics_to_checkpoint:
                if metric_ckpt == metric and big != big_ckpt:
                    logger.error(f"Error! checkpointing and global-logging are in conflict for metric {metric}"
                                 f"- one minimizes and one maximizes.")
                    raise ValueError
        ...
        if params.eval_relations:
            self.relations_path = params.relations_path
        ...
        #init tb writer
        if params.eval_only:
            loggername='runs_eval'
        else:
            loggername='runs_train'
    ...
    def save_best_metrics(self, scores):
        """
        ...
        for metric, biggest in self.metrics:
            ...
            if factor * scores[metric] > factor * self.best_metrics[metric]:
                ...
                #if specified, save the best checkpoint for each metric
                for ckpt_metric, _ in self.metrics_to_checkpoint:
                    if metric == ckpt_metric:
    ...
    def end_epoch(self, scores):
        ...
        End the epoch.
        """
        # log eval metrics to tensorboard or wandb
        for metric, score in scores.items():
            if self.params.log_to_wandb:
                wandb.log({metric: score}, step=self.epoch)
                # log the loss under the same tab as train as well.
                if ("loss" in metric) and ("valid" in metric):
                    wandb.log({"Loss/valid": score}, step=self.epoch)
            else:
                self.writer.add_scalar(metric, score, self.epoch)
                # log the loss under the same tab as train as well.
                if ("loss" in metric) and ("valid" in metric):
                    self.writer.add_scalar("Loss/valid", score, self.epoch)
    ...
    def enc_dec_step(self, task):
        ...
        #add the loss to the logger
        if self.params.log_to_wandb:
            wandb.log({"Loss/train": loss.item()}, step=self.epoch)
        else:
