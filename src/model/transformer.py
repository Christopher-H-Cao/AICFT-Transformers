...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, dim, src_dim, dropout, normalized_attention, xav_init=False, do_rel_pos=None):
        super().__init__()
        ...
        self.do_rel_pos = do_rel_pos
        if do_rel_pos:
            self.linear_pos = nn.Linear(dim, dim, bias=False)
            self.pos_bias_u = nn.Parameter(torch.zeros(n_heads, dim // n_heads))
            self.pos_bias_v = nn.Parameter(torch.zeros(n_heads, dim // n_heads))
    ...
    def forward(self, input, pos_embs, mask, kv=None, use_cache=False, first_loop=True, return_scores=False):
        """
        ...
        if self.do_rel_pos == "rel_pos_qk":
            scores = self._apply_relative_embeddings(q, k, pos_embs)
        elif self.do_rel_pos == "xPos":
            q,k = pos_embs.rotate_queries_and_keys(q,k)
            scores = torch.matmul(q, k.transpose(2, 3))  # (bs, n_heads, qlen, klen)
        elif self.do_rel_pos == "ROPE":
            q = pos_embs.rotate_queries_or_keys(q)
            k = pos_embs.rotate_queries_or_keys(k)
            scores = torch.matmul(q, k.transpose(2, 3))  # (bs, n_heads, qlen, klen)
        else:
            scores = torch.matmul(q, k.transpose(2, 3))  # (bs, n_heads, qlen, klen)
    ...
    def _apply_relative_embeddings(
        ...
        """
        Calculate attention weight with relative position by using "skew algorithm trick".
        Taken from Conformer: (https://github.com/HarunoriKawano/Conformer/blob/main/model/self_attention.py)
        Args:
        ...
        """
        # `(B, L, D)` -> `(B, H, D/H, L)`
        proj_relative_position_embeddings = relative_position_embeddings
        proj_relative_position_embeddings = proj_relative_position_embeddings.view(
            relative_position_embeddings.size(0), -1, self.n_heads, self.head_size
        )
...
class TransformerLayer(nn.Module):
    def __init__(self, params, is_encoder, gated=False, rel_pos_in_layer=None):
        """
        ...
        self.self_attention = MultiHeadAttention(
            ...
            do_rel_pos=rel_pos_in_layer
        )
        self.layer_norm1 = nn.LayerNorm(self.dim, eps=1e-12)
        #do not add rel_pos in cross-attention!
        if self.is_decoder:
    ...
    def forward(self, x, pos_embs, attn_mask, src_mask, src_enc, use_cache=False, cache=None, loop_count=1, return_scores = False):
        tensor = x
        for i in range(loop_count):
            # self attention
            self.self_attention.cache = cache
            if return_scores:
                attn, scores = self.self_attention(tensor, pos_embs, attn_mask, use_cache=use_cache, first_loop=(i == 0),
                                 return_scores = return_scores)
            else:
                attn = self.self_attention(tensor, pos_embs, attn_mask, use_cache=use_cache, first_loop=(i == 0),
                         return_scores = return_scores)
            ...
            if self.is_decoder and src_enc is not None:
                ...
                if return_scores:
                    attn, scores = self.cross_attention(tensor, pos_embs, src_mask, kv = src_enc, use_cache = use_cache, first_loop = (i == 0), return_scores = return_scores)
                else:
                    attn = self.cross_attention(tensor, pos_embs, src_mask, kv = src_enc, use_cache = use_cache, first_loop = (i == 0), return_scores = return_scores)
                attn = F.dropout(attn, p=self.dropout, training=self.training)
                output = output + attn
...
class AdaptiveHalt(nn.Module):
    def __init__(self, params, is_encoder, gated, rel_pos_in_layer):
        super().__init__()
        self.dim = params.enc_emb_dim if is_encoder else params.dec_emb_dim  # 512 by default
        ...
        self.layer = TransformerLayer(params, is_encoder, gated, rel_pos_in_layer)
        self.return_scores = (params.plot_attn_maps and params.eval_only) #bugged on training for some tasks

    def forward(self, input, pos_embs, attn_mask, src_mask, src_enc, use_cache, cache, loop_count):
        bs = input.size(0)
        slen = input.size(1)
        shape = (bs, slen)
        ...
        acc_state = 0
        #only return attn maps for last loop, if looped
        for i in range(self.max_loops):
            ...
            # running at step end
            still_running = torch.less_equal(halting_probability + p * still_running,
            ...
            if self.return_scores:
                input, scores = self.layer.forward(input, pos_embs, attn_mask, src_mask, src_enc, use_cache, cache, loop_count,
                                                   self.return_scores)
            else:
                input = self.layer.forward(input, pos_embs,attn_mask, src_mask, src_enc, use_cache, cache, loop_count,
                                           self.return_scores)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, id2word, is_encoder, with_output):
        ...
        self.onevec_enc = params.onevec_enc
        self.rel_pos = params.rel_pos
        ...
        self.export_input_embs = (params.eval_only and params.export_encoding)
        assert (not self.act) or (self.loop_idx >= 0)

        # embeddings
        if self.rel_pos == "ROPE":
            self.position_embeddings = RotaryEmbedding(self.dim // self.n_heads, use_xpos=False,learned_freq=params.learned_freq)
        elif self.rel_pos == "xPos":
            self.position_embeddings = RotaryEmbedding(self.dim // self.n_heads, use_xpos=True,learned_freq=params.learned_freq)
        else: self.position_embeddings = Embedding(N_MAX_POSITIONS, self.dim)
        ...
        for layer_id in range(self.n_layers):
            ...
            if self.rel_pos=="rel_pos_qk" and layer_id == 0 and self.is_encoder:
                rel_pos_in_layer="rel_pos_qk"
            elif self.rel_pos=="xPos" and layer_id == 0 and self.is_encoder: rel_pos_in_layer="xPos"
            elif self.rel_pos=="ROPE" and layer_id == 0 and self.is_encoder: rel_pos_in_layer="ROPE"
            else: rel_pos_in_layer=None

            if self.act and layer_id == self.loop_idx:
                self.layers.append(AdaptiveHalt(params, self.is_encoder, gated, rel_pos_in_layer=rel_pos_in_layer))
            else:
                self.layers.append(TransformerLayer(params, self.is_encoder, gated, rel_pos_in_layer=rel_pos_in_layer))
    ...
    def fwd(
        ...
        src_mask = None
        if self.is_decoder and src_enc is not None:
            if self.onevec_enc: masksize = 1
            else: masksize = src_len.max()
            src_mask = (
                torch.arange(masksize, dtype=torch.long, device=lengths.device)
                < src_len[:, None]
            )
        ...
        if (self.rel_pos == "ROPE" or self.rel_pos == "xPos"):
            pos_embs = self.position_embeddings
            tensor = embs
        elif self.rel_pos == None:
            pos_embs = self.position_embeddings(positions).expand_as(embs)
            tensor = embs + pos_embs
        else:
            pos_embs = self.position_embeddings(positions).expand_as(embs)
            tensor = embs
        ...
        for i in range(self.n_layers):
            ...
            if self.loop_idx == -2 or self.loop_idx == i:
                loops = self.loops
            if self.return_scores:
                    tensor, scores = self.layers[i].forward(tensor, pos_embs, attn_mask, src_mask, src_enc, use_cache=use_cache,
                                                      cache=self.cache, loop_count=loops, return_scores=self.return_scores)
                    self.all_scores.append(scores)
            else:
                tensor = self.layers[i].forward(tensor, pos_embs, attn_mask, src_mask, src_enc, use_cache=use_cache,
                                                cache = self.cache, loop_count = loops, return_scores = self.return_scores)
        ...
        if self.return_scores:
            #stack the scores and put batch first
            self.all_scores = torch.transpose(torch.stack(self.all_scores),0,1)
        ...
        if self.is_encoder and self.onevec_enc:
            tensor=torch.unsqueeze(tensor[0], dim=0)

        if self.export_input_embs:
            return embs, tensor
        else: return tensor
    ...
    def predict(self, tensor, pred_mask, y, get_scores):
        ...
        if self.act:
            loss = loss + self.layers[self.loop_idx].ponder_penalty
        return scores, loss
    ...
    def generate(self, src_enc, src_len, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            if self.export_input_embs: embs, tensor = output
            else: tensor = output
