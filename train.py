...
def get_parser():
    ...
    #log to wandb
    parser.add_argument("--log_to_wandb", type=bool_flag, default=True,
                        help="log to wandb")
    #log to wandb
    parser.add_argument("--resume_wandb", type=bool_flag, default=False,
                        help="auto-resume wandb")
    ...
    # model parameters
    parser.add_argument("--rel_pos", type=str, choices=['rel_pos_qk', 'ROPE', 'xPos', None], default=None,
                        help="Use relative position encoding instead of absolute")
    parser.add_argument("--learned_freq", type=bool_flag, default=False,
                        help="do learned freqs for rope or not")
    parser.add_argument("--enc_emb_dim", type=int, default=256,
                        help="Encoder embedding layer size")
    parser.add_argument("--onevec_enc", type=bool_flag, default=False,
                        help="Only pass one of the encoder vectors to the decoder")
    ...
    parser.add_argument("--architecture", type=str, default="encoder_decoder",
                        help="model architecture: encoder_decoder, encoder_only, decode_only")
    parser.add_argument("--registers", type=int, default=0,
                        help="number of registers")
    parser.add_argument("--append_registers", type=bool_flag, default=False,
                        help="Registers at end of sequence")
    ...
    parser.add_argument("--reload_data", type=str, default="",
                        help="Load dataset from the disk (task1,train_path1,valid_path1,test_path1;"
    ...
    parser.add_argument("--full_sentence", type=bool_flag, default=False,
                        help="Should the input dict be read all at once (i.e. x->y) or word by word? (i.e. y1x1y2x2...)")
    ...
    parser.add_argument("--eval_verbose_dict", type=bool_flag, default=False,
                        help="Output eval_verbose as a dict. If false, output in line-by-line form")
    parser.add_argument("--eval_verbose_print", type=bool_flag, default=False,
                        help="Print evaluation details")

    #interpretability
    parser.add_argument("--export_encoding", type=bool_flag, default=False,
                        help="Export encoder embeddings to a tsv file. Don't upload to wandb, just store locally")
    parser.add_argument("--plot_attn_maps", type=bool_flag, default=False,
                        help="Plot attention maps")
    parser.add_argument("--examples_to_plot", type=int, default=10,
                        help="Num examples to plot attention maps for")

    # whether to eval relations
    parser.add_argument("--eval_relations", type=bool_flag, default=False,
                        help="eval relations or not?")
    parser.add_argument("--relations_path", type=str, default=None,
                        help="list of relations jsons to eval")
    parser.add_argument("--relations_output", type=str, default="relations_output",
                        help="where to output the relations eval")
    parser.add_argument("--hardcode_trivial_zeroes", type=bool_flag, default=False,
                        help="When we see a trivial zero in a relation, manually set it to zero")
...
def main(params):
    ...
    if params.eval_only:
        ...
        for k, v in scores.items():
            ...
            if params.log_to_wandb:
                wandb.log({k: v}, step=0)
            else:
                trainer.writer.add_scalar(k, v, 0)
    ...
    while trainer.epoch < params.max_epoch:
        ...
        # end of epoch
        trainer.save_best_metrics(scores)
        trainer.save_periodic()
        trainer.end_epoch(scores)

    #print best metrics
    tbmetrics={}
    for (metric, val) in dict(trainer.best_metrics).items():
        tbmetrics["best_"+metric] = val
    logger.info("============ Best global values of each metric ============")
    logger.info(tbmetrics)

    if not params.log_to_wandb:
        # add input params to TB as hparams. Need at least one metric here as well or TB will not show these
        tbparams = dict(vars(params))
        tbparams["tasks"]=",".join([str(item) for item in dict(vars(params))["tasks"]])
        trainer.writer.add_hparams(tbparams,tbmetrics)
    else:
        wandb.save(os.path.join(params.dump_path, "*checkpoint*"))
        wandb.save(os.path.join(params.dump_path, "*best*"))
...
if __name__ == '__main__':
    ...
    for arg in vars(params): aux_parser.add_argument('--' + arg)
    ...
    if params.eval_only and params.eval_from_exp != "":
        ...
        for p in params.__dict__:
            # if the arg is passed explicitly, continue
            if p in cli_args: continue
            #else, overwrite with the args from the pickle
            if p in pickled_args:
                params.__dict__[p] = pickled_args[p]

        params.eval_only = True
        params.reload_model = params.eval_from_exp + '/best-' + params.best_metric_to_load + '.pth'
