...
cp -r runcft_package.tar .
tar -xzvf runcft_package.tar

# This block is for the exit-driven checkpointing. Leave it alone!
...
#change to run directory to actually run the trainiing
cd ChromoBoot_CFT
#move your data from the directory it lives in to the run directory, if it's not there already.
mkdir cft_data
cp -r {PATH TO YOUR DATA} ./cft_data


timeout 4h python train.py --reload_data cfts,./cft_data/{DATA_FILE}.train,./cft_data/{DATA_FILE}.valid,./cft_data/{DATA_FILE}.valid --max_epoch 20 --n_enc_layers $2 --n_dec_layers $2 --num_workers 1 --eval_verbose 1 --batch_size 512 --batch_size_eval 10 --enc_emb_dim 512 --dec_emb_dim 512 --amp 1 --fp16 True --epoch_size 900  --resume_wandb True --env_base_seed $3 --exp_id $4 
timeout_exit_status=$?
