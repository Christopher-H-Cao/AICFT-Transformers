...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, use_cache=False, first_loop=True, return_scores=False):
        """
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, attn_mask, src_mask, src_enc, use_cache=False, cache=None, loop_count=1, return_scores=False):
        tensor = x
        for i in range(loop_count):
            # self attention
            self.self_attention.cache = cache
            if return_scores:
                attn, scores = self.self_attention(tensor, attn_mask, use_cache=use_cache, first_loop=(i==0),
            ...
            if self.is_decoder and src_enc is not None:
                ...
                if return_scores:
                    attn, scores = self.cross_attention(
                        tensor, src_mask, kv=src_enc, use_cache=use_cache, first_loop=(i==0), return_scores=return_scores
                    )
                else:
...
class AdaptiveHalt(nn.Module):
    def __init__(self, params, is_encoder, gated):
        ...
        self.return_scores = (params.return_scores and params.eval_only) #bugged on training for some tasks
    def forward(self, input, attn_mask, src_mask, src_enc, use_cache, cache, loop_count):
        ...
        for i in range(self.max_loops):
            ...
            if self.return_scores:
                input, scores = self.layer.forward(input, attn_mask, src_mask, src_enc, use_cache, cache, loop_count, self.return_scores)
            else:
...
class TransformerModel(nn.Module):
    ...
    def fwd(
        ...
        for i in range(self.n_layers):
            ...
            if self.loop_idx == -2 or self.loop_idx == i:
                loops = self.loops
            if self.return_scores:
                tensor,scores = self.layers[i].forward(tensor, attn_mask, src_mask, src_enc, use_cache=use_cache, cache=self.cache, loop_count=loops,return_scores=self.return_scores)
                self.all_scores.append(scores)
            else:
                tensor = self.layers[i].forward(tensor, attn_mask, src_mask, src_enc, use_cache=use_cache,
    ...
    def get_ref_embedding(
        ...
        """
        Get the tensor of embeddings that looks like x, but with all meaningful tokens padded out. For captum.
        Inputs:
            `x` LongTensor(slen, bs), containing word indices
            `lengths` LongTensor(bs), containing the length of each sentence
            `causal` Boolean, if True, the attention is only done over previous hidden states
      ...
      """
        ...
        if src_enc is not None:
            ...
            assert src_enc.size(0) == bs
        assert not (use_cache and self.cache is None)
        x_dummies = x.detach().clone()
        x_dummies[x_dummies > 1] = 1
